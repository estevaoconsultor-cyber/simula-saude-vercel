...
import { getOwnNetworkProducts, getCredentialsNetworkProducts } from "@/data/network-structure";

type NetworkType = "propria" | "credenciada";

// Usar estrutura imutável da rede
const REDE_PROPRIA = getOwnNetworkProducts().map(p => ({ id: p.id, name: p.name, description: p.description }));
const REDE_CREDENCIADA = getCredentialsNetworkProducts().map(p => ({ id: p.id, name: p.name, description: p.description }));
